...
class Adafruit_CharLCD(object):
    ...
    def __init__(self, pin_rs=23, pin_e=24, pins_db=[12, 16, 20, 21], GPIO=None):
    ...
    def message(self, text, clearFirst = False):
        ...
        line = text[:21] # take first 20 chars only
        for char in line:
            #if char == '\n':
    ...
    def messages(self, textArray, clearFirst = False):
        """ Send string to LCD. Newline wraps to second line"""
        # Clear the screen
        self.write4bits(self.LCD_CLEARDISPLAY)  # command to clear display
        self.delayMicroseconds(3000)  # 3000 microsecond sleep, clearing the display takes a long time
        ...
        for count in range(1,5):
            ...
            if row > self.numlines:
                row = self.numlines - 1  # we count rows starting w/0
            self.write4bits(self.LCD_SETDDRAMADDR | (0 + self.row_offsets[row]))
